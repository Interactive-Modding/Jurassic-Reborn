...
/*
 * MonorailEntity – 1.19.2 RE‑PORT  ✧  2nd pass
 * --------------------------------------------------
...
public class MonorailEntity extends VehicleEntity {
    ...
    // === DATA PARAMETERS ===================================================
    private static final EntityDataAccessor<Boolean> ON_RAILS = SynchedEntityData.defineId(MonorailEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public MonorailEntity(EntityType<? extends VehicleEntity> type, Level level) {
        super(type, level);
    ...
    private void handleRailDetection() {
        ...
        if (!railTracks.equals(prevRailTracks)) {
            Network.sendToAllNear(level, blockPosition(), new MonorailUpdatePositionStateMessage(getId(), rail));
        }
...
//    }
    ...
    protected void doBlockCollisions() {
        if (!isOnRails()) {
            MonorailEntity.this.doBlockCollisions();
        }
    }
    ...
    public class MinecartLogic {
        ...
        public void tick() {
            ...
            MonorailEntity.this.spawnWheelParticles();
            if (getY() < -64) discard();
        ...
        private void moveMinecartOnRail(double mx, double mz) {
            ...
            MonorailEntity.this.move(MoverType.SELF, new Vec3(mx, 0, mz));
        }
