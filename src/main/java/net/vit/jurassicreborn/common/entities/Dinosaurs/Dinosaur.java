...
public abstract class Dinosaur implements Comparable<Dinosaur> {
    ...
    private String scientific_name = "Unknown";
    ...
    public boolean isCarnivore() {
        ...
        for (Diet.DietModule module : this.diet.getModules()) {
            if (module.getFoodType() == FoodType.MEAT) {
                return true;
            }
        }
        return false;
    ...
    protected TabulaModelContainer parseModel(String growthStage) {
        String formattedName = this.getName().toLowerCase(Locale.ENGLISH).replaceAll(" ", "_");
        String modelPath = "/assets/jurassicreborn/models/entities/" + formattedName + "/" + growthStage + "/" + formattedName + "_" + growthStage + "_idle";

        try {
            return TabulaModelHelper.loadTabulaModel(modelPath);
        } catch (Exception e) {
            JurassicReborn.getLogger().fatal("Couldn't load model " + modelPath, e);
        }
    ...
    //todo, should be resourcelocation friendly
    ...
    protected String getDinosaurTexture(String subtype) {
        ...
        String texture = JurassicReborn.MODID+":textures/entities/" + dinosaurName + "/" + dinosaurName;
    ...
    public double getScaleInfant(boolean skeleton) {
        return skeleton ? this.getSkeletonScaleInfant() : this.scaleInfant;
    }
    ...
    public String getLocalizationName() {
        return "entity.JurassicReborn." + this.getName().toLowerCase(Locale.ENGLISH).replaceAll(" ", "_") + ".name";
    }
