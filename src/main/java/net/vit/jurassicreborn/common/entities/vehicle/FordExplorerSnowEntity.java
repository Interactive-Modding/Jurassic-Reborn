...
public class FordExplorerSnowEntity extends VehicleEntity {
    ...
    // === DATA PARAMETERS ===================================================
    private static final EntityDataAccessor<Boolean> ON_RAILS = SynchedEntityData.defineId(FordExplorerSnowEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public FordExplorerSnowEntity(EntityType<? extends VehicleEntity> type, Level level) {
        super(type, level);
    ...
    private void handleRailDetection() {
        ...
        if (!railTracks.equals(prevRailTracks)) {
            Network.sendToAllNear(level, blockPosition(), new FordExplorerSnowUpdatePositionStateMessage(getId(), rail));
        }
...
//    }
    ...
    protected void doBlockCollisions() {
        if (!isOnRails()) {
            FordExplorerSnowEntity.this.doBlockCollisions();
        }
    }
    ...
    public class MinecartLogic {
        ...
        public void tick() {
            ...
            allWheels.forEach(FordExplorerSnowEntity.this::processWheel);
            FordExplorerSnowEntity.this.spawnWheelParticles();
            if (getY() < -64) discard();
        ...
        private void moveMinecartOnRail(double mx, double mz) {
            ...
            FordExplorerSnowEntity.this.move(MoverType.SELF, new Vec3(mx, 0, mz));
        }
