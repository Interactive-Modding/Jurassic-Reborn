...
public class VariantOreFeature extends Feature<OreConfiguration> {
    ...
    public boolean place(FeaturePlaceContext<OreConfiguration> ctx) {
        ...
        var prop = firstState.getBlock()
                .getStateDefinition()
                .getProperty("variant");
        if (!(prop instanceof IntegerProperty variantProp))
            throw new IllegalStateException("Missing variant property on " + firstState);

        int min = variantProp.getPossibleValues().stream().min(Integer::compare).orElse(0);
        int max = variantProp.getPossibleValues().stream().max(Integer::compare).orElse(min);

        List<OreConfiguration.TargetBlockState> variants = new ArrayList<>();
        for (var baseTarget : original.targetStates) {
