...
//            printAllBonePaths();
    ...
    public static RegistryObject<BlockItem> registerBlockItem(String name, Supplier<Block> blockSupplier) {
        return MOD_ITEMS.register(name, () -> {
            ...
            if (tab != null) {
                TabHandler.addToTab(tab, () -> new ItemStack(item));
            }
            return item;
        });
    }
    ...
    private static CreativeModeTab determineTab(Block block) {
        ...
        if (block instanceof FossilBlock) return TabHandler.FOSSILS;
        if (block instanceof ParkBenchBlock || block instanceof TrashCanBlock) return TabHandler.DECORATIONS;
        if (block instanceof SkullDisplayBlock ||block instanceof CultivatorTopBlock) return null;
