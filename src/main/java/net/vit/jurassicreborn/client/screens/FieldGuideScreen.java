...
public class FieldGuideScreen extends Screen {
    ...
    public FieldGuideScreen() {
        super(new TextComponent("InGen Field Guide"));
    ...
    public void render(PoseStack poseStack, int mouseX, int mouseY, float partialTicks) {
        ...
        if (currentIndex >= 0 && currentIndex < dinosaurs.size()) {
            ...
            List<FormattedCharSequence> nameLines = font.split(new TextComponent((currentIndex + 1) + ". " + dino.getName()), wrapWidth);
            for (int i = 0; i < nameLines.size(); i++) {
                font.draw(poseStack, nameLines.get(i), xLeft, y + i * 10, 0x3E3E3E);
            }
            font.draw(poseStack, "Scientific Name:", xLeft, y + 30, 0x3E3E3E);

            List<FormattedCharSequence> sciLines = font.split(new TextComponent(dino.getScientificName()), wrapWidth);
            ...
            List<FormattedCharSequence> periodLines = font.split(new TextComponent("Period: " + dino.getPeriod()), wrapWidth);
            for (int i = 0; i < periodLines.size(); i++) {
                font.draw(poseStack, periodLines.get(i), xLeft, offsetY + 30 + i * 10, 0x3E3E3E);
            }

            List<FormattedCharSequence> familyLines = font.split(new TextComponent("Family: " + dino.getFamily()), wrapWidth);
            for (int i = 0; i < familyLines.size(); i++) {
                font.draw(poseStack, familyLines.get(i), xLeft, offsetY + 50 + i * 10, 0x3E3E3E);
            }

            List<FormattedCharSequence> locationLines = font.split(new TextComponent("Location: " + dino.getLocation()), wrapWidth);
