...
public class MuralRenderer extends EntityRenderer<MuralEntity> {

    public MuralRenderer(EntityRendererProvider.Context ctx) {
        super(ctx);
    }

    @Override
    public void render(MuralEntity entity, float entityYaw, float partialTicks,
                       PoseStack poseStack, MultiBufferSource buffer, int packedLight) {
        poseStack.pushPose();

        // Match painting/item frame rotation
        ...
        // Texture for this mural
        ResourceLocation tex = entity.getMuralType().texture;
        VertexConsumer vc = buffer.getBuffer(RenderType.entityCutoutNoCull(tex));

        // Draw extruded textured layer
        renderLayer(entity,
                entity.getWidth(), entity.getHeight(),
                entity.getMuralType().sizeX, entity.getMuralType().sizeY,
                poseStack, vc);
    ...
    @Override
    public ResourceLocation getTextureLocation(MuralEntity entity) {
        return entity.getMuralType().texture;
    }

    private void renderLayer(MuralEntity entity,
                             int width, int height, int textureWidth, int textureHeight,
                             PoseStack poseStack, VertexConsumer vc) {

        float centerWidth  = -textureWidth  / 2.0F;
        float centerHeight = -textureHeight / 2.0F;
        float pixelSize = 0.0625F; // 1 px = 1/16 block in this scheme
        float depth = 1.0F;

        // Move the front face to z = -depth + 0.5 to match the old orientation
        ...
        for (int x = 0; x < textureWidth * pixelSize; x++) {
            for (int y = 0; y < textureHeight * pixelSize; y++) {
                ...
                for (float i = minX; i < maxX; i += 1.0F) {
                    ...
                    // LEFT (-X)
                    addQuad(vc, mat, nMat, i,     minY, 0.0F,  uMax, maxTextureY,  -1, 0, 0, lightSide);
                    addQuad(vc, mat, nMat, i,     minY, depth, uMax, maxTextureY,  -1, 0, 0, lightSide);
                    addQuad(vc, mat, nMat, i,     maxY, depth, uMin, minTextureY,  -1, 0, 0, lightSide);
                    addQuad(vc, mat, nMat, i,     maxY, 0.0F,  uMin, minTextureY,  -1, 0, 0, lightSide);

                    // RIGHT (+X)
                    addQuad(vc, mat, nMat, i + 1, minY, 0.0F,  uMax, maxTextureY,   1, 0, 0, lightSide);
                    addQuad(vc, mat, nMat, i + 1, minY, depth, uMax, maxTextureY,   1, 0, 0, lightSide);
                    addQuad(vc, mat, nMat, i + 1, maxY, depth, uMin, minTextureY,   1, 0, 0, lightSide);
                    addQuad(vc, mat, nMat, i + 1, maxY, 0.0F,  uMin, minTextureY,   1, 0, 0, lightSide);
                }
                ...
                for (float i = minY; i < maxY; i += 1.0F) {
                    ...
                    int lightTB = computePackedLightForPixel(entity, (minX + maxX) / 2.0F, i);

                    // BOTTOM (-Y)
                    addQuad(vc, mat, nMat, minX, i,     0.0F,  maxTextureX, vMax + 0.5F,  0, -1, 0, lightTB);
                    addQuad(vc, mat, nMat, minX, i,     depth, maxTextureX, vMin + 0.5F,  0, -1, 0, lightTB);
                    addQuad(vc, mat, nMat, maxX, i,     depth, minTextureX, vMin + 0.5F,  0, -1, 0, lightTB);
                    addQuad(vc, mat, nMat, maxX, i,     0.0F,  minTextureX, vMax + 0.5F,  0, -1, 0, lightTB);

                    // TOP (+Y)
                    addQuad(vc, mat, nMat, minX, i + 1, 0.0F,  maxTextureX, vMax + 0.5F,  0, 1, 0, lightTB);
                    addQuad(vc, mat, nMat, minX, i + 1, depth, maxTextureX, vMin + 0.5F,  0, 1, 0, lightTB);
                    addQuad(vc, mat, nMat, maxX, i + 1, depth, minTextureX, vMin + 0.5F,  0, 1, 0, lightTB);
                    addQuad(vc, mat, nMat, maxX, i + 1, 0.0F,  minTextureX, vMax + 0.5F,  0, 1, 0, lightTB);
                }
            }
        }
    }
    ...
    private int computePackedLightForPixel(MuralEntity entity, float xzOffset, float yOffset) {
