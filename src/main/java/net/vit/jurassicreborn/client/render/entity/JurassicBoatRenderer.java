...
public class JurassicBoatRenderer<T extends net.minecraft.world.entity.vehicle.Boat> extends EntityRenderer<T> {
    ...
    public JurassicBoatRenderer(EntityRendererProvider.Context context) {
        super(context);
        this.shadowRadius = 0.8F;

        this.boatResources = Stream.of(ModBoatType.values()).collect(Collectors.toMap(
                type -> type,
                type -> Pair.of(createTexture(type), createModel(context, type)),
                (a, b) -> a,
    ...
    private ResourceLocation createTexture(ModBoatType type) {
        return new ResourceLocation(JurassicReborn.MODID, "textures/entity/boat/" + type.getSerializedName() + ".png");
    }

    private BoatModel createModel(EntityRendererProvider.Context context, ModBoatType type) {
        ModelLayerLocation layer = JurassicBoatModelLayers.createBoatModelName(type);
        ModelPart part = context.bakeLayer(layer);

        return new BoatModel(part);
    }
