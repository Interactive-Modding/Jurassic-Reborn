...
public class CearadactylusAnimator extends EntityAnimator<CearadactylusEntity> {
    ...
    @Override
    protected void performAnimations(AnimatableModel model, CearadactylusEntity entity, float f, float f1, float ticks, float rotationYaw, float rotationPitch, float scale) {
        ...
        if (entity.isOnGround() && !entity.isCarcass()) {
            ...
            if (!entity.isCarcass()) {
                ...
                // slight shoulder drop for wings
                leftArm1.offsetY += 0.35F;
                rightArm1.offsetY += 0.35F;

                // moving vs idle hover
                ...
                if (movingAnim) {
                    ...
                    // --- idle hover flap (ticks-driven) ---
                    float s = 0.21f; // flap speed
                    float a = 0.62f; // flap amplitude
                    ...
                    // continuous wing flap using ticks (works when f1 == 0)
