...
public class CamerocerasAnimator extends EntityAnimator<CamerocerasEntity> {

    protected void performAnimations(AnimatableModel parModel, CamerocerasEntity entity, float limbSwing, float limbSwingAmount, float ticks, float rotationYaw, float rotationPitch, float scale) {
        AdvancedModelBox shell = parModel.getCube("Shell segment 8");
        if (shell != null) {
            shell.rotateAngleY += (float) Math.PI;
        }
