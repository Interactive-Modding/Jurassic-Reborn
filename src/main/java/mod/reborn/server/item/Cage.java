...
import mod.reborn.server.entity.item.CageEntity;
            ...
            if (itemStackIn.hasTagCompound() && itemStackIn.getTagCompound().hasKey("EntityTag")) {
                ...
                if (result != null && result.typeOfHit == RayTraceResult.Type.ENTITY) {
                    ...
                    if (entity instanceof EntityLivingBase && !(entity instanceof EntityPlayer)) {
                        // Capture the entity
                        CageEntity.captureEntity((EntityLivingBase) entity, itemStackIn);
                        return new ActionResult<>(EnumActionResult.SUCCESS, itemStackIn);
                    }
                }
            }
    ...
    public void addInformation(ItemStack stack, @Nullable World worldIn, List<String> tooltip, ITooltipFlag flagIn) {
        if (stack.hasTagCompound()) {
            if (stack.getTagCompound().hasKey("name")) {
                tooltip.add(TextFormatting.GREEN + I18n.format("tooltip.cage.stored") + stack.getTagCompound().getString("name"));
            } else {
                tooltip.add(TextFormatting.RED + I18n.format("tooltip.cage.stored") + stack.getTagCompound().getString("id"));
            }
        } else {
            tooltip.add(TextFormatting.RED + I18n.format("tooltip.cage.stored") + I18n.format("tooltip.cage.empty"));
        }
    }
}
